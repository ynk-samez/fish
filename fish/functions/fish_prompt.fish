...
set -q color_git_dirty_bg; or set -g color_git_dirty_bg white
set -q color_git_dirty_str; or set -g color_git_dirty_str black
set -q color_git_bg; or set -g color_git_bg red
...
function prompt_status -d "the symbols for a non zero exit status, root and background jobs"
    if [ $RETVAL -ne 0 ]
      prompt_segment $color_status_nonzero_bg $color_status_nonzero_str "ERROR ✖︎"
    end
...
function fish_prompt
  set -g RETVAL $status
  prompt_segment yellow black  " "\uE0A2" yunoki "
  prompt_status
  prompt_virtual_env
  prompt_user
  prompt_dir
